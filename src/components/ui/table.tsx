...
>(({ className, ...props }, ref) => (
  <div className="relative w-full overflow-auto">
    <table
      ref={ref}
      className={cn('w-full caption-bottom text-sm rounded-lg', className)} // Added rounded-lg here for table border-radius
      {...props}
    />
  </div>
));
...
>(({ className, ...props }, ref) => (
  <tr
    ref={ref}
    className={cn(
      'transition-colors border mt-3 rounded-lg data-[state=selected]:bg-neutral-100  dark:data-[state=selected]:bg-neutral-800', // rounded-lg for the row
      className
    )}
    {...props}
  />
